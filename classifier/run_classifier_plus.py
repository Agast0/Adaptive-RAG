...
from utils import *
from utils_plus import calculate_accuracy_perClass_plus
...
def main():
    ...
    if args.do_eval:
                ...
                probs = (
                    torch.nn.functional.softmax(
                        torch.stack([
                            ...
                            scores[:, tokenizer('D').input_ids[0]],
                        ]), dim=0,
                    ).detach().cpu().numpy()
                )
        ...
        # Acc per class (4-way A/B/C/D variant)
        final_eval_results_perClass = calculate_accuracy_perClass_plus(gold_answers, predictions)
